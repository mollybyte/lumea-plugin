...
- **Проект** (`list_projects`): `id`, `name`, `type` — имена и типы брать из ответа этого инструмента, не угадывать. Фильтры тикетов берут `projectName` — как в справочнике; внешние сервисы (`get_client_external`, `run_external_action`) — `projectType`; `find_similar_tickets`, `list_quick_replies`, `staff_metrics` — `projectId`.
